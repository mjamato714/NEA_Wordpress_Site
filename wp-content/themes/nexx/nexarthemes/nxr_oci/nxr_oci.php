<?php
/*
	Extension Name: NexarThemes OneClickInstall
	Plugin URI: http://nexarthemes.com/
	Author: NexarThemes
	Author URI: https://nexarthemes.com
	Version: 2.0.0
	Description: It executes the OneClick install of NXR Demos configured in import_config.php
	Text Domain: nexx
	...
	class NXR_OCI {
		...
		*/
		public function __construct(){	
			...
			$this->this_dir			=	get_template_directory('/nexarthemes/nxr_oci/');
			
			// Include necesarry files
			$this->configFile		=	include get_template_directory() . '/nexarthemes/nxr_oci/import_config.php';
		...
		*/
		function oci_menu() {
			add_theme_page( 'OneClick Demo Install', 'OneClick Demo', 'manage_options', 'nxr_oci_install', array($this,'display_demos') );
		}
		...
		*/
		function display_demos() {
				...
				$return ='<div class="wrap"><h2>'.esc_html( $theme->get( 'Name' ) ).' '.esc_html__( 'OneClick Demo Setup', 'nexx' ).'</h2>';
				
				if( is_array( $this->configFile ) && !is_null( $this->configFile ) ){
					$return .='<p><div class="import_message">'.esc_html__( 'Click on the desired demo to install demo content and theme options setup.', 'nexx' ).'</div><p>';
					$return .='<p><input type="checkbox" name="resetall" id="resetall" value="true"> '.esc_html__( 'Reset WordPress Database? This will erase all content (posts, pages, etc) and install demo content.', 'nexx' ).'<p>';
					...
					foreach( $this->configFile as $demo => $demoContent ) {
						$return .='<div class="demo_container" style="float:left;margin:20px;text-align:center;max-width:315px;background-color:#fff">
							<img src="' . esc_url( trailingslashit( get_template_directory_uri() ) . 'nexarthemes/nxr_oci/gfx/' . $demoContent['demoImg'] ) . '" style="width:100%; height:auto;"><br>
							<h4>' . esc_html( $demoContent['demoName'] ).'</h4>
							<a href="' . esc_url( $demoContent['liveURL'] ) . '" target="_blank">
							<button style="margin:5px;background-color:#fff;border:1px solid #dedede;cursor:pointer;color:#000;padding:10px;width:143px;">'.esc_html__( 'PREVIEW', 'nexx' ).'</button>
							</a> 
							<button class="bootstrapguru_import" data-install="' . esc_attr( $demo ) . '" data-reset="false" style="margin:5px;background-color:#5194ec;border:0;cursor:pointer;color:#fff;padding:10px;width:143px;">'.esc_html__( 'INSTALL', 'nexx' ).'</button>
							</div>';
					}
				
					$return .='</div>';
				} else {
					$return .='<p><div class="import_message">'.esc_html__( "It looks like the config file for the demos is missing or conatins errors!. Demo install can\'t go futher!", 'nexx' ).'</div><p>';
				}
				$return .='</div>';
		...
		function importSliderRev($slidersArray){
			if ( class_exists( 'RevSlider' ) ) {
				foreach($slidersArray as $slider){
					...
					$sliderFile = $this->this_dir.'/nexarthemes/nxr_oci/demo_files/revSlider/'.$slider;
		...
		*	Import Essential Grid
		*/
		function importEssGrid($essGridFileArray){
			if ( class_exists( 'Essential_Grid_Import' ) ) {
				foreach($essGridFileArray as $essGridFile){
					$json_file = wp_remote_get($this->this_uri.'/nexarthemes/nxr_oci/demo_files/essGrid/'.$essGridFile);
					if( $json_file['response'][code] == '200' && !empty($json_file['body']) ){
						...
						return esc_html__( 'Essential Grids could not be imported.', 'nexx' ).'<br>';
					}
				}
			} else {
				return esc_html__( 'It looks like you don\'t have Essential Grid installed and activated. Grids were not imported!', 'nexx' ).'<br>';
			}
		}
		...
		*/
		function importDemoContent($demoXml){
			...
			if ( !class_exists( 'WP_Import' ) ) {
				$class_wp_importer = get_template_directory() ."/nexarthemes/nxr_oci/wordpress-importer.php";
				if ( file_exists( $class_wp_importer ) )
					require $class_wp_importer;
			}
			...
			if ( class_exists( 'WP_Import' ) ) {
				$import_filepath = $this->this_dir.'/nexarthemes/nxr_oci/demo_files/content/'.$demoXml; // Get the xml file from directory 
				...
				$blog = get_page_by_path('blog-standard');
				if ($blog) {
					update_option('show_on_front', 'page');
		...
		*/
		function importThemeOptions($themeOptionsFile){
			$json_file = wp_remote_get($this->this_uri.'/nexarthemes/nxr_oci/demo_files/themeOptions/'.$themeOptionsFile);
			if( $json_file['response']['code'] == '200' && !empty($json_file['body']) ){
				if(update_option( 'redux_options', json_decode($json_file['body'], true), '', 'yes' )){
					echo esc_html__( 'Theme Options saved.', 'nexx' ).'<br>';
				}
			} else{
				echo esc_html__( 'Theme Options could not be imported', 'nexx' ).'<br>';
			}
		}
		...
		*/
		function load_backends(){
			wp_register_script('nxr_oci', $this->this_uri.'/nexarthemes/nxr_oci/nxr_oci.js', array('jquery'), '2.0.0', true );
			wp_localize_script( 'nxr_oci', 'this_dir', $this->this_uri.'/nexarthemes/nxr_oci/' );
			wp_enqueue_script('nxr_oci');
		}
