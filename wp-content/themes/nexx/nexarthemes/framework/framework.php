...
// Fix for the GT3 page builder: http://www.gt3themes.com/wordpress-gt3-page-builder-plugin/
    ...
    if ( ! class_exists( 'ReduxFrameworkInstances' ) ) {
        // Instance Container
        require_once get_template_directory() . '/nexarthemes/framework/inc/class.redux_instances.php';
        require_once get_template_directory() . '/nexarthemes/framework/inc/lib.redux_instances.php';
    }
...
// Don't duplicate me!
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        // Redux CDN class
        require_once get_template_directory() . '/nexarthemes/framework/inc/class.redux_cdn.php';

        // Redux API class  :)
        require_once get_template_directory() . '/nexarthemes/framework/inc/class.redux_api.php';

        // General helper functions
        require_once get_template_directory() . '/nexarthemes/framework/inc/class.redux_helpers.php';

        // General functions
        require_once get_template_directory() . '/nexarthemes/framework/inc/class.redux_functions.php';
        require_once get_template_directory() . '/nexarthemes/framework/inc/class.p.php';

        require_once get_template_directory() . '/nexarthemes/framework/inc/class.redux_filesystem.php';

        require_once get_template_directory() . '/nexarthemes/framework/inc/class.redux_admin_notices.php';

        // ThemeCheck checks
        require_once get_template_directory() . '/nexarthemes/framework/inc/themecheck/class.redux_themecheck.php';

        // Welcome
        require_once get_template_directory() . '/nexarthemes/framework/inc/welcome/welcome.php';
        ...
        class ReduxFramework {
            ...
            public static function init() {
                $dir = Redux_Helpers::cleanFilePath( get_template_directory() . '/nexarthemes/framework/' );
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'nexx' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . esc_html__( 'Redux Framework', 'nexx' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = esc_html__( 'Options', 'nexx' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = esc_html__( 'Options', 'nexx' );
                }
...
// __construct()
            ...
            private function _internationalization() {
                 ...
                 * @param string     'nexx'  text domain
                 */
                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'nexx' );
                ...
                if ( ReduxFramework::$_is_plugin ) {
                    load_plugin_textdomain( 'nexx', '', self::$_dir . 'languages/' . $locale . '.mo' );
                } else {
                    load_textdomain( 'nexx', self::$_dir . 'languages/' . $locale . '.mo' );
                }
...
// get_instance()
            ...
            private function _tracking() {
                if ( file_exists( get_template_directory() . '/nexarthemes/framework/inc/tracking.php' ) ) {
                    require_once get_template_directory() . '/nexarthemes/framework/inc/tracking.php';
...
// _enqueue_output()
            ...
            public function _enqueue() {
                get_template_part( 'nexarthemes/framework/core/enqueue' );
...
// _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = esc_html__( 'Enable', 'nexx' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = esc_html__( 'Disable', 'nexx' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = esc_html__( 'moving the mouse over', 'nexx' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = esc_html__( 'clicking', 'nexx' );
                    }

                    // Construct message
                    $msg = sprintf( esc_html__( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'nexx' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => esc_html__( 'Hints', 'nexx' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
...
// admin_footer_text()
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = esc_html__( 'Default', 'nexx' ) . ": " . substr( $default_output, 0, - 2 );
                }
...
// _register_settings()
            ...
            private function _register_extensions() {
                $path    = get_template_directory() . '/nexarthemes/framework/inc/extensions/';
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => esc_html__( 'Invalid security credential.  Please reload the page and try again.', 'nexx' ),
                        'action' => ''
                    ) );
                ...
                if ( ! current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => esc_html__( 'Invalid user capability.  Please reload the page and try again.', 'nexx' ),
                        'action' => ''
                    ) );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => esc_html__( 'Your panel has no fields. Nothing to save.', 'nexx' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                ...
                if ( isset( $return_array ) ) {
                    if ( $return_array['status'] == "success" ) {
                        get_template_part( 'nexarthemes/framework/core/panel' );
                            ...
                            if (is_array($arr) && !empty($arr)) {
                                foreach($arr as $x => $y) {
                                    if (strpos(strtolower($y), 'redux') >= 0) {
                                        $msg = esc_html__('<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>admin_bar_links</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'nexx');
                    ...
                    if (!empty($this->args['share_icons'])) {
                        foreach($this->args['share_icons'] as $idx => $arr) {
                            if (is_array($arr) && !empty($arr)) {
                                foreach($arr as $x => $y) {
                                    if (strpos(strtolower($y), 'redux') >= 0) {
                                        $msg = esc_html__('<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>share_icons</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'nexx');
