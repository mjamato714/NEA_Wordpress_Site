...
<div id="nxr_top_navbar_container" class="nxr_navbar <?php echo ( !is_front_page() ? '' : ( $nxr_options['header_floating'] == 2 ? 'hidden' : '') ); ?>">
    ...
    <div id="nxr_top_navbar_extras" class="">
    	...
    	<?php 
		  ...
		  <div class="nxr_woo_minicart sage-cart-icon <?php echo esc_attr($hideonmobile);?>"><span class="helper"></span>
			<div class="woo_bubble"><a class="nxr_woo_minicart_content" href="<?php global $woocommerce; echo esc_url($woocommerce->cart->get_cart_url()); ?>" title="<?php esc_html_e('View your shopping cart', 'nexx'); ?>">
			<?php echo sprintf(_n('%d', '%d', $woocommerce->cart->cart_contents_count, 'nexx'), $woocommerce->cart->cart_contents_count);?></a><?php echo ( class_exists('NXR_QCV') ? do_shortcode( '[nxr_quick_cart]' ) : '' );?></div>
		  </div>
